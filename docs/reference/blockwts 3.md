# blockwts (DATA) population weights of Census blocks
...
that block. The weights are used to aggregate block-level data to
blockgroup, for cases where only some of the blockgroup is in a circular
buffer or polygon.
