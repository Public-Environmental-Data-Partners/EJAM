# Flexibly read .csv or .xlsx of lat/lon points or facility IDs WORK IN PROGRESS THAT WAS DRAFTED

Flexibly read .csv or .xlsx of lat/lon points or facility IDs WORK IN
PROGRESS THAT WAS DRAFTED
...
so one can read in a file or table or vectors - any format - and the
other figures out if it is latlon or program IDs or registry IDs (but
not NAICS?) and I guess turns those into lat, lon, siteid as below.
...
would be the most general / flexible broadest way to get points, but is
