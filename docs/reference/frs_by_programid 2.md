...
frs_by_programid (DATA) data.table of Program System ID code(s) for each
...
This is the format with one row per site-programid pair, so multiple
rows for one site if it is in multiple programs.
