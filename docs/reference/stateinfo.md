# stateinfo (DATA) data.frame of state abbreviations and state names (50+DC+PR; not AS, GU, MP, VI, UM)

52 rows and a few variables: ST is the 2-letter abbreviation, statename
is the State name, etc.

## Usage

``` r
stateinfo
```

## Format

An object of class `data.frame` with 52 rows and 5 columns.

## Details

Created for EJAM by datacreate_stateinfo.R script

Also see [Census
details](https://www.census.gov/programs-surveys/decennial-census/decade/2020/planning-management/release/2020-island-areas-data-products.html)

column names: 'ST' 'statename' 'ftpname' 'FIPS.ST' 'REGION'

Some datasets lack PR. (72)

Many datasets lack these: AS, GU, MP, VI (codes '60' '66' '69' '78')

Almost all datasets lack UM. (74)

        72 PR                 Puerto Rico

        66 GU                        Guam

        69 MP    Northern Mariana Islands

        78 VI         U.S. Virgin Islands

        74 UM U.S. Minor Outlying Islands

## See also

[stateinfo2](https://public-environmental-data-partners.github.io/EJAM/reference/stateinfo2.md)
for more columns
