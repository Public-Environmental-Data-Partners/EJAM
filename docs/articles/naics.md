...
EJAM helps select regulated sites based on industrial classification,
...
- Note that a very large fraction of all FRS sites (as obtained for use
  in EJAM) lack NAICS code!

- Note that EJAM may query FRS sites differently than the FRS search
  tool or other query tools would.
...
- Note that searching on a 6-digit code misses parent categories you may
  ...
  are reported for the NAICS (explained below).

A given site might be listed in the FRS as being under one or more NAICS
codes of various lengths, such as only a parent code (large grouping),
only a detailed code (6-digit), or some combination of codes and their
subcategories.
