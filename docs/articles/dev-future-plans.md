...
EJAM API is needed. This would allow links within the EJAM tables and
map popups to link to a report on each site, for example, which was
available via the EJSCREEN API until it was taken offline. An API also
could provide access to services such as fast identification of nearby
blocks or other points, summarization (aggregation over blocks or
blockgroups), or data. See draft work in the plumber folder of the EJAM
source package.
