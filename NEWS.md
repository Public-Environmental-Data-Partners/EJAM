...
- Bug fixes:
  - Fixed issue where States could not be analyzed in the web app.
  - Fixed issue where size of circular buffer at each point on map in the report was not reflecting the actual radius.
  - Fixed issue affecting API where a request to find all blockgroups in a city did not work.
...
- add `sites_only()` helper; add `sites_from_input()` examples
- `ejam2map()` gets radius parameter
- added helper functions to make columns of averages, ratios to avg, and percentiles (can use later to replace parts of `doaggregate()`)
