...
#   EJAM:::calc_acs_by_geography(), EJAM:::calc_ejscreen_threshold_layers(),
#   EJAM:::calc_ejscreen_threshold_layers_from_exports(), and
...
test_that("calc_acs_by_geography excludes Island Areas by default (identified by ST)", {
  ...
  inc <- suppressMessages(EJAM:::calc_acs_by_geography(bg, levels = "blockgroup", exclude_islandareas = FALSE))$blockgroup
  exc <- suppressMessages(EJAM:::calc_acs_by_geography(bg, levels = "blockgroup", exclude_islandareas = TRUE))$blockgroup
...
test_that("calc_acs_by_geography sums counts and uses per-indicator (denominator) weights", {
  ...
  tr <- suppressMessages(EJAM:::calc_acs_by_geography(bg, levels = "tract", exclude_islandareas = FALSE))$tract
...
test_that("calc_acs_by_geography does not mutate a data.table input and honors id_col", {
  ...
  nb <- ncol(dt)
  invisible(suppressMessages(EJAM:::calc_acs_by_geography(bg = dt, levels = "state", exclude_islandareas = FALSE)))
  expect_equal(ncol(dt), nb)  # no leaked ..num../..wgt.. helper columns

  r <- suppressMessages(EJAM:::calc_acs_by_geography(
...
test_that("calc_acs_by_geography repairs a numeric FIPS column before deriving parent GEOIDs", {
  ...
  r <- suppressMessages(EJAM:::calc_acs_by_geography(bg, levels = c("blockgroup", "tract", "state")))
...
test_that("calc_ejscreen_threshold_layers computes P1..P100 hit counts (NA-aware, rounded)", {
  ...
  L <- EJAM:::calc_ejscreen_threshold_layers(
...
test_that("calc_ejscreen_threshold_layers_from_exports builds the four layers", {
  ...
  res <- EJAM:::calc_ejscreen_threshold_layers_from_exports(nat, st)
  ...
  res_us <- EJAM:::calc_ejscreen_threshold_layers_from_exports(nat, NULL)
...
test_that("ejscreen_compare_geography_to_epa maps names, scales pct, keeps already-matching, ignores unmapped", {
  ...
  cmp <- EJAM:::ejscreen_compare_geography_to_epa(ejam, epa)
