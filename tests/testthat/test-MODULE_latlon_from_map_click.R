...
test_that("MODULE_SERVER_latlon_from_map_click respects the `enabled` gate (shared-map case)", {
  # When an outer app shares one map across modes, the module must not accumulate/remove points
  # while its mode is inactive - and enabling it must not replay the last (ignored) click.
  rd <- shiny::reactiveVal(data.frame(lat = numeric(0), lon = numeric(0)))
  ac <- shiny::reactiveVal(NULL)
  rc <- shiny::reactiveVal(NULL)
  en <- shiny::reactiveVal(FALSE)  # this mode is NOT active

  shiny::testServer(
    EJAM:::MODULE_SERVER_latlon_from_map_click,
    args = list(id = "TESTID", reactdat = rd,
                add_click = ac, remove_click = rc, clear = shiny::reactiveVal(NULL),
                render_map = FALSE, enabled = en),
    {
      # clicks while disabled are ignored (no hidden accumulation)
      ac(list(lat = 40, lng = -99)); session$flushReact()
      expect_equal(NROW(reactdat()), 0)

      # enabling must NOT replay the last (ignored) click
      en(TRUE); session$flushReact()
      expect_equal(NROW(reactdat()), 0)

      # a fresh click while enabled is added
      ac(list(lat = 34.05, lng = -118.25)); session$flushReact()
      expect_equal(NROW(reactdat()), 1)
      expect_equal(reactdat()$lat, 34.05)

      # disabling again ignores further clicks
      en(FALSE); session$flushReact()
      ac(list(lat = 25, lng = -80)); session$flushReact()
      expect_equal(NROW(reactdat()), 1)
    }
  )
})
