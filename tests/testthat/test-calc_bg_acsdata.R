...
test_that("calc_bg_acsdata can save a validated bg_acsdata stage", {
  ...
  expect_true(file.exists(file.path(pipeline_dir, "bg_acsdata.rds")))
  expect_equal(ejscreen_pipeline_load("bg_acsdata", pipeline_dir, format = "rds"), out)
  expect_true(all(c("pctpre1960", "pctdisability", "disab_universe") %in% names(out)))
})

test_that("download_bg_acs_raw saves blockgroup and tract raw ACS tables", {
  ...
  expect_true(file.exists(file.path(pipeline_dir, "bg_acs_raw.rds")))
  expect_equal(ejscreen_pipeline_load("bg_acs_raw", pipeline_dir, format = "rds")$yr, 2024)
})
