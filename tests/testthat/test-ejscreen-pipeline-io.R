...
test_that("envirodata stage validation requires pctpre1960", {
  ...
  expect_error(
    ejscreen_pipeline_save(missing_lead_paint, "envirodata", pipeline_dir, format = "rds"),
    "pctpre1960"
  )
  ...
  path <- ejscreen_pipeline_save(envirodata, "envirodata", pipeline_dir, format = "rds")
  expect_true(file.exists(path))
  expect_equal(ejscreen_pipeline_load("envirodata", pipeline_dir, format = "rds"), envirodata)
})
