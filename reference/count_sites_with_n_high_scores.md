...
This function provides tables of summary stats but also text that
explains those findings in plain English.
...
- thresholds:
  ...
  thresholds vector of numbers as benchmarks. Assuming the indicators in
  the scores table are ratios to the average, then the thresholds could
  be for example, 1.50, 2, etc. which would represent ratios that are
  1.5x or 2x etc.

- indicator_type:

  Scores and benchmarks are percentiles, ratios, or anything else. One
  of these: c("percentile", "ratio", "other")

- xwide:

  must be "statewide" or "nationwide" – used only in the text output
  that describes the findings.

- site_stat_type:

  Count or share of sites, exactly or cumulatively. One of these:
  c("cum_pct", "cum_count", "pct", "count")

- text_suffix:

  If using ratios, use the default, which explains these thresholds as X
  times the average. If using percentiles as thresholds, set text_suffix
  = "." or text_suffix = "th percentile in the state." for example

- text_indicatortype:

  can be "Summary Indexes" "EJ Indexes" etc.
...
Returns a list with two named elements, "stats" and "text" where stats
is a 3-dimensional array of numbers. See dimnames(output\$stats).
...
x <- data.table::setDF(data.table::copy(x))
