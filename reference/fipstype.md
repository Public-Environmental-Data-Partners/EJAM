...
FIPS - Identify what type of Census geography each FIPS code seems to be
(block, county, etc.)
...
vector of types: "block", "blockgroup", "tract", "city", "county", or
"state"
...
leading zero. An 11-character string starting in 3, 7, or 0 cannot be a
bg missing a leading zero, so could only be a tract (if valid). Since a
12-character bg fips never starts with 03, 07, or 00.
