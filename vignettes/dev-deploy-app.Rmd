...
IMPORTANT: Most of this info is obsolete but is retained here to document how this used to be done. The EPA-hosted app had a Posit Connect server configured to re-publish the shiny app directly from github.com content whenever it saw changes in the specified repo and branch. The notes below about updating the `manifest.json` file using a script like `rsconnect_manifest_update.R` might still be relevant to someone hosting the app.
...
## Customizing the Defaults and Settings

See the article [Defaults and Custom Settings for the Web App](dev-app-settings.html) for more details.
