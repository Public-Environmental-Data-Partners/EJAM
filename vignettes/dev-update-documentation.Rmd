...
-   Articles (called vignettes here) are written and edited as `.Rmd` files in
    the `vignettes` folder. Preview one article before rebuilding the whole
    site with, for example:
...
publish anything and do not require committing generated HTML.
...
The web-based documentation pages are built from source files and published by
GitHub Actions. Edit and commit source files such as `.R`, `.Rmd`, `.Rd` inputs,
`README.Rmd`, `NEWS.md`, and `_pkgdown.yml`. Do not commit generated `docs/`
HTML from source branches.
...
-   For local checks, `EJAM:::pkgdown_update()` is still available as a
    ...
    publishing workflow. For a faster local site preview without publishing:
...
-   Public publishing is handled by `.github/workflows/pkgdown.yaml`. GitHub
    ...
    branch. The source branches should not publish from `main/docs`.
