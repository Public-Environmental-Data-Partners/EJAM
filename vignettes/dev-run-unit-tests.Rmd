...
late 2025, so ideally more unit tests would be added in those cases.
...
That function also has an option that helps check coverage -- that relies on
**`EJAM/tests/test_coverage_check.R`** which is imperfect but somewhat helpful,
and its source code has notes on the topic. The `test_ejam()` function noted
above already prompts for using that helper.

Note that it is a bit difficult to figure out which functions have unit tests since
...
Also see `devtools::test_coverage()` which is a shortcut for `covr::package_coverage()` plus `covr::report()`. Note the [covr package](https://covr.r-lib.org/) also has a related RStudio addin.
...
-   if the web app UI changes - then special tests of the web app functionality
    can be updated/created as explained in [Testing EJAM App with
    shinytest2](dev-run-shinytests.html). Also note that any screenshots for any
    version of an [EJAM web app User Guide](ejscreen.html#help-with-how-to-use-ejscreen) that are included in the package would need to be updated if the UI changes, and any unit tests that rely on screenshots or other aspects of the UI would need to be updated if the UI changes.

### Running unit tests

There are numerous unit tests in the `EJAM/tests/testthat/` folder that you can
run and add to as needed. 

The unit tests can be run automatically by the `devtools::test()` function.
...
development branch. See `EJAM/.github/workflows/test-webapp-functionality.yml`
