...
That function also has an option that helps check coverage. It relies on
...
use it. The source code there also had notes on the topic.
...
Also see `devtools::test_coverage()` which is a shortcut for `covr::package_coverage()` plus `covr::report()`.
The [covr package](https://covr.r-lib.org/) also has a related RStudio addin.
...
development branch. See `EJAM/.github/workflows/test-shiny-web-app-functionality.yml`
