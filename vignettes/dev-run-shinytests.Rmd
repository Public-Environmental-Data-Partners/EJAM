...
-   **`shinytest2_webapp_functionality.R`** – Contains generic function, `shinytest2_webapp_functionality()`, that defines app interactions for testing, running tests for multiple data types (FIPS, shapefile, latlon, NAICS, etc.).
...
Modify `shinytest2_webapp_functionality.R` to add new interactions with the app for the shinytest to test.
Note there was a copy of that function in setup.R as well!
...
-   The workflow is stored in `.github/workflows/test-shiny-web-app-functionality.yml`.
...
-   Add `print()`, `message()`, or `warning()` statements in `shinytest2_webapp_functionality.R`.
-   Run, line-by line or in chunks, the main shinytest code in `shinytest2_webapp_functionality.R` beginning with:
