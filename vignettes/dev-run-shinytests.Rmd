...
-   **`testthat/setup.R`** – Does setup (loads `global_*.R` and app scripts into the testing environment, etc.), and sources `shinytest2_webapp_functionality()` which is in `testthat/setup-shinytest2.R`

-   **`setup-shinytest2.R`** – Contains source code for shinytest2_webapp_functionality() which does a series of tests of web app UI interactions, using the app to upload or select, and analyze, multiple data types (FIPS, shapefile, latlon, NAICS, etc.).

-   **`testthat/test-webapp-[DATA TYPE]-functionality.R`** – Each of these just does the call to shinytest2_webapp_functionality(), specifying the data type to test with.
