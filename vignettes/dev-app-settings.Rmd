...
### ejamapp() params or global_defaults_

`ejamapp()` calls get_global_defaults_or_user_options() which
gets the global defaults and then overrides them with
any user-specified options given as arguments in ejamapp().
ejamapp() then launches the app, storing those options as app$appOptions$golem_options
...
- in global_defaults_ or param in `ejamapp()`  default_upload_dropdown, which is the initial selected value of
