...
matching the one-method-per-analysis model. These set per-session reactives that
`data_up_latlon()`, `data_up_fips()`, and `data_up_shp()` prefer over the
`ejamapp()`/global defaults, so the app opens with those sites selected and ready
to run. See the handler near the top of the SELECT SITES section in `app_server.R`.
