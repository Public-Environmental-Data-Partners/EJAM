...
- Datasets stored within the EJAM package (.rda files): [Documentation](../reference/index.html#datasets-with-indicators-raw-data-means-percentiles-) and [access to data files](`r paste0(EJAM:::repo_from_desc(get_full_url = TRUE), "/tree/main/data")`)

- Datasets used by EJAM but stored separately (large .arrow files): [Documentation](../reference/index.html#datasets-with-indicators-raw-data-means-percentiles-) and [access to data files](`r paste0(EJAM:::repo_from_desc(get_full_url = TRUE,  domain = "github.io"), "/tree/main/data")`)
