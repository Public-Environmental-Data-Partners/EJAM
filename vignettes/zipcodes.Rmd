...
The simplest way to analyze zip codes is to pass them directly to `ejamit()`,
which converts them to ZCTA polygons via `shapes_from_zip()` and analyzes them
like any other shapefile (the first use downloads a large national boundaries
file, which is slow, but it is cached locally so later uses are fast):

```{r ejamit_zipcode, eval=FALSE, include=TRUE}
...
The rest of this article shows the manual, step-by-step version of that
workflow, which offers more control (e.g., other sources of zip code polygons).
