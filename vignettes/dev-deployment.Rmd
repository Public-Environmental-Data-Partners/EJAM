...
PDF support is checked at three different layers because a source-only test cannot
detect a broken container configuration:
...
1. **During the image build:** the package `configure` script performs a real
   ...
   `EJAM_PDF_BUILD_SMOKE_TEST=true`). It verifies both the output size and `%PDF`
...
2. **In the Shiny browser suite:** the full `latlon` case requests both PDF and
   ...
   pass.
3. **After deployment:** `Smoke-test deployed PDF` runs after either successful
...
The post-deploy smoke workflow lives on the default branch, as required by
