...
app_server <- function(input, output, session) {
  ...
  orig_leaf_map <- reactive({
    ...
    if ("SHP" %in% current_upload_method()) {
      ...
      ## Check how many valid points and if > max can map or analyze
      max_pts_map <- input$max_pts_map # was the fixed max_pts_map
      max_pts_run <- input$max_pts_run
       ...
       ## Max allowed points FOR MAP was exceeded!
        if (valid_pts_count > max_pts_run) {
          ...
          msg <- paste0('Too many valid points (',
                        prettyNum(valid_pts_count, big.mark = ","), '> ',
                        prettyNum(max_pts_run, big.mark = ','),
                        ') uploaded for analysis')
          cat(msg, "\n")
          validate(msg) # halts map creation
        } else {
          ## too many to map but not too many to analyze
          msg <- paste0('Too many valid points (',
                        prettyNum(valid_pts_count, big.mark = ","), '> ',
                        prettyNum(max_pts_map, big.mark = ','),
                        ') uploaded to map all, so only showing a random sample, but you can still run the analysis on all.')
          showNotification(msg) # warns, does not halt reactive
          cat(msg, "\n")
          # show just a subset up to max_pts_map allowed ***
          sampling_of_valid_rows <- sample(1:valid_pts_count, max_pts_map)
          data_tomap <- data_uploaded()[valid, ][sampling_of_valid_rows, ]
        }
      } else {
