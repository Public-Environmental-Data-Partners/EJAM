...
#'
#' @examples
#'  out1 <- testoutput_ejamit_10pts_1miles$results_bysite
#'
...
#'
#'  plot_boxplot_ratios(calc_ratios_to_avg(out1)$ratios_d,"pctlowinc","% low income",
...
calc_ratios_to_avg <- function(out,
  ...
  if (all(names_d_subgroups %in% dvarnames)) {
    if (zone.prefix == "") {
      if (!(all(names_d_subgroups_avg %in%  names(out)) && all(names_d_subgroups %in%  names(usastats)))) {
        message("US averages for demog subgroups like pcthisp not found
        so looking up and adding those, and rescaling as 0-100, which is how ejscreen tables stored percentages though not how ejamit() does")
    ...
    if (zone.prefix == "state.") {
      if (!all(names_d_subgroups_state_avg %in% names(out)) && all(names_d_subgroups %in% names(statestats))) {
        ...
        if ("ST" %in% names(out)) {
          ...
          message("State averages for demog subgroups like pcthisp not found
        so looking up and adding those, and rescaling as 0-100, which is how ejscreen tables stored percentages though not how ejamit() does")
  ...
  # so blanks wont cause error, make any non numeric into NA
