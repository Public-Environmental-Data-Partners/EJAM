...
# datacreate_blockgroup_pctdisability or calc_blockgroup_pctdisability() -  TAKES A WHILE TO DOWNLOAD THE CENSUS DATA FOR EVERY STATE !!
...
calc_blockgroup_pctdisability <- function(yr)  {
  ...
  for (i in 1:length(stateinfo$ST)) {
    c2k[[i]] <- tidycensus::get_decennial(state = stateinfo$ST[i],
