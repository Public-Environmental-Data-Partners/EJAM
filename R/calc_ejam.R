...
# calc_varname_from_formula()  - utility to get the names of variables created by the formulas
#   vector of formulas as text strings like "a = 1 + b"
...
#  store formulas for indicators as a table of metadata but also
#  want to allow user to specify a custom formula provided as text, to create a new indicator or summary stat
#  (and EJAM could then aggregate and report the new stat alongside the built-in indicators).
######################################## #
## Historical note:
...
#'
#' - [calc_byformula()] uses [calc_varname_from_formula()] and maybe source_this_codetext()
#'
...
calc_ejam <- function(bg,
  ...
  if (is.null(formulas)) {
    if (exists("formulas_ejscreen_acs")) {
      formulas <- formulas_ejscreen_acs$formula
    } else {
  ...
  if (keep.new[1] == 'all') {
    keep.new <- calc_varname_from_formula(myformulas) # tries to keep all of those formulas would try to create
  }
...
#'
#' @param myforms see [calc_byformula()] and [formulas_ejscreen_acs]$formula
#'
#' @inherit calc_ejam details
#'
#' @examples
...
#' @keywords internal
#'
calc_varname_from_formula <- function(myforms) {
