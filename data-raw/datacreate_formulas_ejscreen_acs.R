...
#
# This script was  edited/ used 5/2026 to update and save the formulas,
# but would not be used again in this form for annual updates of formulas
...
# tables_ejscreen_acs is this package's list of the ACS tables that are used to create EJSCREEN indicators, and thus are the tables that need to be downloaded and processed to create the EJSCREEN indicators.
{
  # > cbind(tables_ejscreen_acs)
  # tables_ejscreen_acs
  # [1,] "B25034"
  # [2,] "B01001"
  # [3,] "B03002"
  ...
  # [15,] "B18101"
  # > tables_ejscreen_acs
  # [1] "B25034" "B01001" "B03002" "B02001" "B15002" "B23025" "C17002" "B19301" "B25032" "B28003" "B27010" "C16002" "B16004" "C16001" "B18101"
...
# GETTING INFO ON ACS TABLES VIA tidycensus::load_variables ####

# BUT NOTE AS OF LATE APRIL 2026 THAT PACKAGE HAD NOT UPDATED TO INCLUDE ACS2024 GEOGRAPHY TABLE
# NEEDED TO CHECK WHAT RESOLUTION EACH TABLE IS PROVIDED AT.
...
# COPY OF THIS DATASET OBJECT AS SNAPSHOT at one point maybe not when finalized ####
{
  x <-
    structure(list(
      ...
      formula = c("pop = B01001_001", "ageunder5m = B01001_003",
                  ...
                  "age85upf = B01001_049", "hisp = B03002_012", "pop3002 = B03002_001",
                  "nonhisp = B03002_002", "nhwa = B03002_003", "nhba = B03002_004",
                  "nhaiana = B03002_005", "nhaa = B03002_006", "nhnhpia = B03002_007",
                  "nhotheralone = B03002_008", "nhmulti = B03002_009", "povknownratio = C17002_001",
                  "pov50 = C17002_002", "pov99 = C17002_003", "pov124 = C17002_004",
                  "pov149 = C17002_005", "pov184 = C17002_006", "pov199 = C17002_007",
                  "pov2plus = C17002_008", "age25up = B15002_001", "m0 = B15002_003",
                  "m4 = B15002_004", "m6 = B15002_005", "m8 = B15002_006", "m9 = B15002_007",
                  "m10 = B15002_008", "m11 = B15002_009", "m12 = B15002_010", "f0 = B15002_020",
                  "f4 = B15002_021", "f6 = B15002_022", "f8 = B15002_023", "f9 = B15002_024",
                  "f10 = B15002_025", "f11 = B15002_026", "f12 = B15002_027", "lingisospanish = C16002_004",
                  ...
                  "num1pov <- pov50 + pov99", "num15pov <- num1pov + pov124 + pov149",
                  ...
                  "lowinc = povknownratio - pov2plus", "pctlowinc = ifelse( povknownratio==0, 0, lowinc / povknownratio)",
                  ...
                  "pctflood <- ifelse(pop == 0, 0, as.numeric(flood) / pop)", "pctflood30 <- ifelse(pop == 0, 0, as.numeric(flood30) / pop)",
                  "pctfemale <- ifelse(pop == 0, 0, as.numeric(female) / pop)",
                  "pctmale <- ifelse(pop == 0, 0, as.numeric(male) / pop)", "pctownedunits <- ifelse(occupiedunits == 0, 0, as.numeric(ownedunits) / occupiedunits)",
                  ...
                  "pctaa <- ifelse(pop == 0, 0, as.numeric(aa) / pop)", "pctaiana <- ifelse(pop == 0, 0, as.numeric(aiana) / pop)",
                  "pctba <- ifelse(pop == 0, 0, as.numeric(ba) / pop)", "pctmulti <- ifelse(pop == 0, 0, as.numeric(multi) / pop)",
                  "pctnhpia <- ifelse(pop == 0, 0, as.numeric(nhpia) / pop)", "pctotheralone <- ifelse(pop == 0, 0, as.numeric(otheralone) / pop)",
                  "pctwa <- ifelse(pop == 0, 0, as.numeric(wa) / pop)", "EJ.DISPARITY.pctpre1960.eo <- ifelse(pop == 0, 0, as.numeric(EJ.DISPARITY.pre1960.eo) / pop)",
                  ...
                  "poor = pov50 + pov99", "wa = B02001_002", "ba = B02001_003",
                  "aa = B02001_005", "aiana = B02001_004", "nhpia = B02001_006",
                  "otheralone = B02001_007", "multi = B02001_008"),

      longname_old = c("Total population",
                       "Count of males age Under 5 years", "Count of males age 5 to 9 years",
                       "Count of males age 10 to 14 years", "Count of males age 15 to 17 years",
                       "Count of males age 65 and 66 years", "Count of males age 67 to 69 years",
                       "Count of males age 70 to 74 years", "Count of males age 75 to 79 years",
                       "Count of males age 80 to 84 years", "Count of males age 85 years and over",
                       "Count of females age Under 5 years", "Count of females age 5 to 9 years",
                       "Count of females age 10 to 14 years", "Count of females age 15 to 17 years",
                       "Count of females age 65 and 66 years", "Count of females age 67 to 69 years",
                       "Count of females age 70 to 74 years", "Count of females age 75 to 79 years",
                       "Count of females age 80 to 84 years", "Count of females age 85 years and over",
                       ...
                       NA, NA, NA, NA, NA, NA, NA, NA, "Spanish - Limited English speaking household",
                       ...
                       "Built 1940 to 1949", "Built 1939 or earlier", "Count of denominator for % unemployed",
                       "Count of people unemployed", "count of individuals under age 5",
                       ...
                       "% pre-1960 housing (lead paint indicator)", "% Unemployed",
      ...
      longname = c("Total Population", "Count of males age Under 5 years",
                   ...
                   "Population with income at least twice the poverty level", "Count of Population Age 25 up",
                   "m0", "m4", "m6", "m8", "m9", "m10", "m11", "m12", "f0", "f4",
                   "f6", "f8", "f9", "f10", "f11", "f12", "Spanish - Limited English speaking household",
                   "Other Indo-European languages - Limited English speaking household",
                   "Asian and Pacific Island languages - Limited English speaking household",
                   ...
                   "% Black or African American (non-Hispanic, single race)", "% American Indian and Alaska Native (non-Hispanic, single race)",
                   ...
                   "Estimated Flood Risk in 30 Years", "% Females", "% Males", "% Owner Occupied households",
                   "% Households without Broadband Internet", "% Households without Health Insurance",
                   "% of Households below Poverty Level", "% speaking Asian and Pacific Island languages at home",
                   "% speaking Arabic at home", "% speaking English at home", "% speaking French at home",
                   "% speaking Other Indo-European at home", "% speaking Non English languages at home",
                   "% speaking Other and Unspecified languages at home", "% speaking Other Asian and Pacific Island languages at home",
                   ...
                   "% speaking Spanish (as % of limited English households)", "% Asian (single race, includes Hispanic)",
                   "% American Indian and Alaska Native (single race, includes Hispanic)",
                   ...
                   "% with Disabilities", "bgid", "County name", "State Name", "State Abbreviation",
                   "EPA Region", "Population Under Age 18", "Population Over Age 17",
                   "Female Population", "Male Population", "ownedhhlds", "occupiedhhlds",
                   "pctownedhhlds", "Count of Owner Occupied Housing Units", "Occupied Housing Units",
                   "nobroadband", "nohealthinsurance", "Households below Poverty Level",
                   "Count of White (single race, includes Hispanic)", "Count of Black or African American (single race, includes Hispanic)",
      ...
      varlist = c("names_d_other_count", NA, NA, NA, NA, NA, NA,
                  NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, "names_d_subgroups_count",
                  NA, NA, "names_d_subgroups_count", "names_d_subgroups_count",
                  "names_d_subgroups_count", "names_d_subgroups_count", "names_d_subgroups_count",
                  "names_d_subgroups_count", "names_d_subgroups_count", "names_d_other_count",
                  NA, NA, NA, NA, NA, NA, NA, "names_d_other_count", NA, NA, NA,
                  NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, NA,
                  NA, "names_d_other_count", "names_d_other_count", NA, NA, NA,
                  "names_d_other_count", "names_d_count", "names_d_count", "names_d",
                  "names_d_count", "names_d", "names_d_other_count", "names_d_subgroups",
                  "names_d_subgroups", "names_d_subgroups", "names_d_subgroups",
                  "names_d_subgroups", "names_d_subgroups", "names_d_subgroups",
                  "names_d_subgroups", "names_d_count", "names_d", NA, NA, NA,
                  NA, NA, NA, NA, NA, "names_d_count", "names_d", "names_d_count",
                  "names_d", "names_d_count", "names_d", "names_d_other_count",
                  "names_e", "names_d", "names_age", "names_age", "names_climate",
                  "names_climate", "names_climate", "names_climate", "names_community",
                  "names_community", "names_community", "names_criticalservice",
                  "names_criticalservice", "names_d_extra", "names_d_language",
                  "names_d_language", "names_d_language", "names_d_language", "names_d_language",
                  "names_d_language", "names_d_language", "names_d_language", "names_d_language",
                  "names_d_language", "names_d_language", "names_d_language", "names_d_languageli",
                  ...
                  "names_d_subgroups_alone", "names_d_subgroups_alone", "names_d_subgroups_alone",
                  "names_d_subgroups_alone", "names_ej", "names_ej_state", "names_ej_supp",
                  "names_ej_supp_state", "names_health", NA, "names_geo", "names_geo",
                  "names_geo", "names_geo", "names_age_count", "names_age_count",
                  "names_community_count", "names_community_count", NA, NA, NA,
                  "names_d_extra_count", "names_community", NA, NA, "names_d_extra_count",
      ...
      ejscreen_version = c(VersionEJSCREEN = "2.5.0"),
      ejscreen_releasedate = c(ReleaseDateEJSCREEN = "2026-05-14"),
      acs_releasedate = c(ReleaseDateACS = "2026-01-29"),
      acs_version = c(VersionACS = "2020-2024"),
      census_version = c(VersionCensus = "2020"),
      date_saved_in_package = "2026-04-29",

      row.names = c(NA, 163L), class = "data.frame")
...
## check if the same rname defined more than once ####
...
# ~ ####
  ...
  ### REMOVE DUPLICATES OF rname again now
  ...
  # formulas_ejscreen_acs  Metadata and saved for use in package ####
  ...
  ## now reinstall/build to be sure
  ...
  ################################################## ################################################### #
  # >   cbind(x, duplicated(x))
  # rname                                             formula longname_old     longname varlist duplicated(x)
  # 169 lan_other_ie lan_other_ie = B16004_009 + B16004_031 + B16004_053         <NA> lan_other_ie    <NA>         FALSE
  # 179 lan_other_ie                           lan_other_ie = C16001_015         <NA> lan_other_ie    <NA>         FALSE
...
# >  info_for_map_headernames
# rname                              longname
# 1                  lan_other_ie              People speaking other_ie
# 2                   lan_english               People speaking english
# 3                    lan_french                People speaking french
# 4                    lan_german                People speaking german
# 5                 pctlan_german                     % speaking german
# 6              lan_rus_pol_slav          People speaking rus_pol_slav
# 7                  lan_other_ie              People speaking other_ie
# 8                    lan_korean                People speaking korean
# 9                 pctlan_korean                     % speaking korean
# 10                  lan_chinese               People speaking chinese
# 11               pctlan_chinese                    % speaking chinese
# 12               lan_vietnamese            People speaking vietnamese
# 13              lan_other_asian           People speaking other_asian
# 14                  lan_tagalog               People speaking tagalog
# 15               pctlan_tagalog                    % speaking tagalog
# 16                   lan_arabic                People speaking arabic
# 17    lan_other_and_unspecified People speaking other_and_unspecified
# 18 pctlan_other_and_unspecified      % speaking other_and_unspecified
#################################################################### #################################### #
#
#
# c16001_vars <- data.frame(
...
############ #################################### #
x = tidycensus::load_variables(2024, "acs5")
# x = x[x$geography %in% "block group", ] # BUT THAT EXCLUDES C16001, B18101
x = x[x$geography %in% c("tract", "block group"), ] # INCL C16001, B18101
x$geography <- NULL
x$table = gsub("_.*$", "", x$name)
