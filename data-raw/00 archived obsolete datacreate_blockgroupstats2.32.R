...
# notes saved from old SCRIPT TO DOWNLOAD/UPDATE EJSCREEN BLOCKGROUP DATA AND PERCENTILE LOOKUP TABLES FOR EJAM YEARLY

############################################################################################ #
# # ARCHIVES as of approx 2/1/25:
...
# and then EPA team posted version 2.32 on 8/12/24 here:
...
# date created by epa was 8/2024           date archived seemed to be 2/3/25
#
...
# # as of 8/21/24 2.32 August version filenames that were on ftp site:  (but offline as of Feb 2025)
...
#  locate data for   blockgroupstats that used ACS 2018-2022 data
...
#   data for   usastats and statestats that used ACS 2018-2022 data
...
# if (interactive() && askquestions) {
  savex = utils::askYesNo("Save usastats_new_explained.xlsx in localfolder?")
...
########################################################### #
########################################################### #
# READ back in archived versions ####
...
## ISLAND AREAS archived as .rda, .arrow ####
#
## but drop them from blockgroupstats_new and blockgroupstats_new_state

bg_islandareas <- blockgroupstats_new[                      !EJAM:::fips_valid(blockgroupstats_new$bgfips),
                                                            EJAM:::nacounts(
                                                              blockgroupstats_new[!EJAM:::fips_valid(blockgroupstats_new$bgfips), ],
                                                              showall = T)$other > 0]
require(arrow)
...
# fips2state_abbrev(unique(substr(blockgroupstats_new$bgfips ,1,2))) -- also had AS, GU, MP, VI
...
# > length(unique(blockgroupstats_new$bgfips))
# [1] 243022
# > NROW(blockgroupstats_new )
# [1] 243022
# nacounts(blockgroupstats_new$bgfips)
# nacounts(blockgroupstats_new[,1:20]) # so far bgfips and ST are not NA here at all
...
# > table(nchar(blockgroupstats_new$bgfips))
#   7     10     12
# 270    416 242336   The 686 cases are FIPS of 7 or 10 digits.
# > unique(blockgroupstats_new$ST[nchar(blockgroupstats_new$bgfips) == 10])
# [1] "VI"
# > unique(blockgroupstats_new$ST[nchar(blockgroupstats_new$bgfips) == 7])
# [1] "AS" "GU" "MP"
...
# AS OF 8/14/24 THERE were STILL 19 Connecticut blockgroups FIPS in bgfips
# (derived from the block weights file provided directly by the EJSCREEN team)
#  that were NOT in blockgroupstats (derived from the geodatabase on the FTP site)
# ***
