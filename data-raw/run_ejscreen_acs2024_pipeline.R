...
load_csv_stage <- function(stage) {
  ejscreen_pipeline_load(stage, pipeline_dir = pipeline_dir, format = "csv")
}

stage_exists <- function(stage) {
  ejscreen_pipeline_stage_exists(stage, pipeline_dir = pipeline_dir, format = "csv")
}
...
if (force_bg_acsdata || !file.exists(stage_path("bg_acsdata"))) {
...
if (file.exists(bg_envirodata_path)) {
  ...
  ejscreen_pipeline_save(bg_envirodata, "bg_envirodata", pipeline_dir, format = "csv", overwrite = TRUE)
  writeLines(
    c(
    ...
    con = file.path(pipeline_dir, "bg_envirodata_SOURCE.txt")
  )
} else {
...
if (file.exists(bg_extra_indicators_path)) {
  ...
  writeLines(
    ...
    con = file.path(pipeline_dir, "bg_extra_indicators_SOURCE.txt")
  )
}
...
fwrite(validation_summary, file.path(pipeline_dir, "pipeline_validation_summary.csv"))
